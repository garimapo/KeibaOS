...
_ACCESS_S_URL = _re.compile(
    r"https://www\.jra\.go\.jp/JRADB/accessS\.html\?CNAME="
    r"pw01sde(?:01|10)(?:0[1-9]|10)[0-9]{4}(?:0[1-9]|[1-9][0-9])"
    r"(?:0[1-9]|1[0-2])(?:0[1-9]|1[0-2])(?P<calendar_date>[0-9]{8})%2F[0-9A-F]{2}\Z"
)
...
def _access_s_calendar_date(canonical_source_url: str) -> _date:
    match = _ACCESS_S_URL.fullmatch(canonical_source_url)
    if match is None:
        raise _validation("canonical accessS URL is invalid")
    value = match.group("calendar_date")
    try:
        return _date(int(value[:4]), int(value[4:6]), int(value[6:8]))
    except ValueError as error:
        raise _validation("canonical accessS calendar date is invalid") from error
...
def _validate_visible_header(
    ...
    *,
    ...
    if (
        visible_date != access_s_calendar_date
        or access_s_calendar_date != snapshot.race.target_race_date
...
def normalize_and_persist_jra_target_race_result(
    *,
    ...
    access_s_calendar_date = _access_s_calendar_date(capture.canonical_source_url)
    if access_s_calendar_date != snapshot.race.target_race_date:
        raise _validation("canonical accessS calendar date disagrees with snapshot target race date")

    soup = _document(capture)
    _validate_visible_header(
        soup,
        snapshot=snapshot,
        race_identity=capture_race_identity,
        access_s_calendar_date=access_s_calendar_date,
    )
