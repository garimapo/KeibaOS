"""Version four schema for durable JRA target race-selection POST captures."""
...
_OLD = "jra_official_response_captures_v003"
...
def _time(value: object) -> _datetime:
    if type(value) is not str or len(value) != 32:
        raise RuntimeError("v003 stored timestamp is invalid")
    try:
        ...
        raise RuntimeError("v003 stored timestamp is invalid") from error
...
def _index(connection: _sqlite3.Connection, name: str, columns: tuple[str, ...], predicate: str) -> None:
    ...
    if len(entries) != 3 or len(automatic) != 1 or len(custom) != 2 or len(wanted) != 1 or wanted[0][2:] != (1, "c", 1):
        raise RuntimeError("v003 capture indexes are invalid")
    if tuple(row[2] for row in connection.execute(f"PRAGMA index_info({name})")) != columns:
        raise RuntimeError("v003 evidence index order is invalid")
    ...
    if [(row[0], row[2], row[5]) for row in xinfo] != [(index, column, 1) for index, column in enumerate(columns)] + [(len(columns), "capture_id", 0)]:
        raise RuntimeError("v003 evidence index has expressions or extra keys")
    sql = connection.execute("SELECT sql FROM sqlite_master WHERE type='index' AND name=?", (name,)).fetchone()
    if len(sql or ()) != 1 or _normalized_ddl(sql[0]) != predicate:
        raise RuntimeError("v003 evidence index predicate is invalid")
...
def _rejected(connection: _sqlite3.Connection, sql: str, values: tuple[object, ...]) -> None:
    try:
        ...
        return
    raise RuntimeError("v003 CHECK constraint is not enforced")


def _probe_constraints(connection: _sqlite3.Connection) -> None:
    if connection.execute("PRAGMA foreign_keys").fetchone() != (1,):
        raise RuntimeError("v003 foreign-key enforcement is disabled")
    digest = _hashlib.sha256(b"__jra_v003_probe_body__").hexdigest()
    unknown = _hashlib.sha256(b"__jra_v003_probe_unknown__").hexdigest()
    values = ["__jra_v003_probe_v1__", 1, "race_result", "https://www.jra.go.jp/JRADB/accessS.html?CNAME=pw01sde0106202504030420250913%2FDC", digest, "cp932", "2026-01-01T00:00:00.000000+00:00", "2026-01-01T00:00:00.000000+00:00", "2026-01-01T00:00:00.000000+00:00", 200, "text/html", None, None, None, None, None, "GET", None, None]
    names = _COLUMNS.split(",")
    insert = f"INSERT INTO {_TABLE}({_COLUMNS}) VALUES({','.join('?' for _ in names)})"
    connection.execute("SAVEPOINT jra_v003_schema_probe")
    try:
        ...
        for index, value in ((1, 4), (2, "unknown"), (3, ""), (3, _sqlite3.Binary(b"url")), (4, unknown), (5, "utf8"), (9, 201), (11, "gzip"), (15, -1), (15, "one"), (16, "PUT"), (17, "x" * 64), (18, ""), (6, "2026-01-01T00:00:01.000000+00:00"), (7, "2026-01-01T00:00:01.000000+00:00"), (16, "POST")):
            candidate = list(values)
            candidate[index] = value
            _rejected(connection, insert, tuple(candidate))
        target = list(values)
        target[0] = "__jra_v003_probe_v3__"
        target[1] = 3
        target[2] = "target_race_card"
        target[3] = "https://www.jra.go.jp/JRADB/accessD.html?CNAME=pw01dde0106202504030420250913%2FDC"
        connection.execute(insert, tuple(target))
        for index, value in ((2, "race_result"), (16, "POST"), (17, "0" * 64), (18, "pw01drl00062025040320250403%2FDC")):
            candidate = list(target)
            candidate[0] = f"__jra_v003_probe_v3_{index}__"
            candidate[index] = value
            _rejected(connection, insert, tuple(candidate))
    finally:
        connection.execute("ROLLBACK TO jra_v003_schema_probe")
        connection.execute("RELEASE jra_v003_schema_probe")


def _validate_v003(connection: _sqlite3.Connection) -> None:
    """Fail closed before mutation; v004 never adopts an arbitrary v003 lookalike."""

    for name, expected in _EXPECTED_V003_DDL.items():
        ...
        if len(row or ()) != 1 or _normalized_ddl(row[0]) != expected:
            raise RuntimeError("v003 DDL differs from the approved registered schema")
    if _columns(connection, "jra_official_response_bodies") != _BODY_COLUMNS:
        raise RuntimeError("v003 response-body table columns are invalid")
    ...
    if len(body_indexes) != 1 or body_indexes[0][2:] != (1, "pk", 0):
        raise RuntimeError("v003 response-body indexes are invalid")
    if _columns(connection, _TABLE) != _CAPTURE_COLUMNS:
        raise RuntimeError("v003 capture table columns are invalid")
    ...
    if foreign_keys != [(0, 0, "jra_official_response_bodies", "response_sha256", "response_sha256", "RESTRICT", "RESTRICT", "NONE")]:
        raise RuntimeError("v003 capture foreign key is invalid")
    _index(connection, _EVIDENCE_INDEX, ("canonical_source_url", "response_sha256", "observed_at_utc"), _EXPECTED_V003_DDL[_EVIDENCE_INDEX])
    _index(connection, _REQUEST_INDEX, ("canonical_source_url", "request_identity_sha256", "response_sha256", "observed_at_utc"), _EXPECTED_V003_DDL[_REQUEST_INDEX])
    _probe_constraints(connection)
    invalid = connection.execute("SELECT 1 FROM jra_official_response_captures WHERE (schema_version=1 AND (page_kind NOT IN ('race_result','horse_profile_history') OR request_method<>'GET' OR request_identity_sha256 IS NOT NULL OR request_cname IS NOT NULL)) OR (schema_version=2 AND (page_kind<>'final_win_odds' OR request_method<>'POST' OR request_identity_sha256 IS NULL OR request_cname IS NULL)) OR (schema_version=3 AND (page_kind<>'target_race_card' OR request_method<>'GET' OR request_identity_sha256 IS NOT NULL OR request_cname IS NOT NULL)) OR schema_version NOT IN (1,2,3) LIMIT 1").fetchone()
    if invalid is not None:
        raise RuntimeError("v003 capture row family is invalid")
    ...
    for row in rows:
        ...
        if len(body_rows) != 1:
            raise RuntimeError("v003 capture body is missing or duplicated")
        ...
        if type(body) is not bytes or type(byte_length) is not int or byte_length != len(body) or type(digest) is not str or _hashlib.sha256(body).hexdigest() != digest:
            raise RuntimeError("v003 capture body is invalid")
        try:
            if version == 1:
...
def apply(connection: _sqlite3.Connection) -> None:
    ...
    connection.execute("""CREATE TABLE jra_official_response_captures (
        capture_id TEXT PRIMARY KEY,
        schema_version INTEGER NOT NULL CHECK(typeof(schema_version)='integer' AND schema_version IN (1,2,3,4)),
        page_kind TEXT NOT NULL CHECK(page_kind IN ('race_result','horse_profile_history','final_win_odds','target_race_card','target_race_selection')),
        canonical_source_url TEXT NOT NULL CHECK(typeof(canonical_source_url)='text' AND canonical_source_url<>''),
        response_sha256 TEXT NOT NULL REFERENCES jra_official_response_bodies(response_sha256) ON UPDATE RESTRICT ON DELETE RESTRICT,
        charset TEXT NOT NULL CHECK(charset='cp932'), requested_at_utc TEXT NOT NULL, observed_at_utc TEXT NOT NULL, stored_at_utc TEXT NOT NULL,
        http_status INTEGER NOT NULL CHECK(typeof(http_status)='integer' AND http_status=200), content_type TEXT NOT NULL,
        ...
        request_cname TEXT NULL CHECK(request_cname IS NULL OR (typeof(request_cname)='text' AND length(request_cname)>0)),
        CHECK(requested_at_utc<=observed_at_utc AND observed_at_utc<=stored_at_utc),
        CHECK((schema_version=1 AND page_kind IN ('race_result','horse_profile_history') AND request_method='GET' AND request_identity_sha256 IS NULL AND request_cname IS NULL) OR (schema_version=2 AND page_kind='final_win_odds' AND request_method='POST' AND request_identity_sha256 IS NOT NULL AND request_cname IS NOT NULL) OR (schema_version=3 AND page_kind='target_race_card' AND request_method='GET' AND request_identity_sha256 IS NULL AND request_cname IS NULL) OR (schema_version=4 AND page_kind='target_race_selection' AND request_method='POST' AND request_identity_sha256 IS NOT NULL AND request_cname IS NOT NULL))
    ) WITHOUT ROWID""")
