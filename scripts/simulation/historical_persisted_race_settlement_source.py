"""Cutoff-aware persisted settlement facts for one historical race."""
...
from .persisted_settlement import PersistedRaceSettlementData
...
class HistoricalPersistedRaceSettlementSource:
    """Load settlement facts visible at an exact per-race historical cutoff."""

    __slots__ = (
        "_bet_source",
        "_race_result_repository",
        "_payout_repository",
        "_settlement_cutoffs_by_race_id",
    )
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(settlement_cutoffs_by_race_id, Mapping):
        ...
        self._settlement_cutoffs_by_race_id = cutoffs
    ...
    def load_settlement_data(
        ...
        *,
        ...
        if race_input.race_id not in self._settlement_cutoffs_by_race_id:
            raise SimulationValidationError(
                race_input.race_id,
                "settlement_cutoffs_by_race_id",
                "settlement cutoff was not provided for race_id",
            )
        settlement_cutoff = self._settlement_cutoffs_by_race_id[race_input.race_id]
        ...
        effective_race_result = (
            None
            if race_result is not None and race_result.observed_at > settlement_cutoff
            else race_result
        )
        ...
        for bet_type in required_bet_types:
            ...
            self._validate_payout_publication(
                publication,
                race_input,
                bet_type,
                settlement_cutoff,
        ...
        return PersistedRaceSettlementData(
            race_id=race_input.race_id,
            bets=bets,
            race_result=effective_race_result,
    ...
    def _validate_bets(
        ...
        if type(bets) is not tuple:
            HistoricalPersistedRaceSettlementSource._invalid_bets(
                race_input,
    ...
    def _validate_payout_publication(
        ...
        if publication.observed_at > settlement_cutoff:
            raise SimulationValidationError(
                race_input.race_id,
                "payout_repository",
    ...
    @staticmethod
    def _invalid_bets(race_input: SimulationRaceInput, reason: str) -> None:
