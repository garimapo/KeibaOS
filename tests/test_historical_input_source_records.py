...
class HistoricalInputSourceRecordsTest(unittest.TestCase):
    ...
    def test_public_api_field_contract_and_no_package_export(self) -> None:
        ...
        self.assertEqual(field_map["schema_version"].default, 4)
    ...
    def test_six_exact_schemas_nullable_payloads_and_defensive_freeze(self) -> None:
        expected_keys = {
            ...
            "past_race": {"race_date", "place", "race_name", "race_class", "distance_m", "track", "weather", "track_condition", "finish", "race_time", "weight", "weight_diff", "jockey", "popularity", "odds", "passing_order", "fourth_corner_position"},
        ...
        for kind, keys in expected_keys.items():
            with self.subTest(kind=kind):
                ...
                self.assertTrue(record.source_id.startswith(f"his-v4:{kind}:"))
    ...
    def test_schema_type_key_and_scalar_validation(self) -> None:
        ...
        with self.assertRaises(HistoricalInputSourceValidationError): _record("odds_win", record_values=values)
        for bad in ("", 0, True, Decimal("1.0")):
            with self.subTest(race_time=bad):
                values = _values("past_race"); values["race_time"] = bad
        ...
        with self.assertRaises(HistoricalInputSourceValidationError): _record("past_race", record_values=values)
        values = _values("past_race"); values["reference_time_difference_seconds"] = Decimal("0")
        with self.assertRaises(HistoricalInputSourceValidationError): _record("past_race", record_values=values)
    ...
    def test_text_decimal_datetime_and_temporal_contract(self) -> None:
        ...
        self.assertEqual(canonical_historical_input_source_payload(record=record)["record_values"]["race_time"], "1:32.0")
    ...
    def test_deterministic_payload_id_and_timestamp_exclusion(self) -> None:
        ...
        self.assertTrue(one.source_id.startswith("his-v4:odds_win:"))
        self.assertEqual(len(one.source_id.rsplit(":", 1)[1]), 64)

    def test_v4_past_race_factual_change_isolated_to_its_source_id(self) -> None:
        baseline = (
            _record("track"),
            _record("entry"),
            _record("jockey"),
        ...
        changed_values["race_time"] = "1:32.1"
        ...
        for record in baseline + changed:
            ...
            self.assertTrue(record.source_id.startswith(f"his-v4:{record.record_kind}:"))
        ...
        self.assertEqual(
            {key: value for key, value in before["record_values"].items() if key != "race_time"},
            {key: value for key, value in after["record_values"].items() if key != "race_time"},
        )
        self.assertEqual(before["record_values"]["race_time"], "1:32.0")
        self.assertEqual(after["record_values"]["race_time"], "1:32.1")
