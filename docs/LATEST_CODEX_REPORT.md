...
## Phase 4C-2d3b1i6d1d5f1c4f1 Implementation
...
Implemented the pure keyword-only
`build_simulation_race_input_from_historical_snapshot(...)` boundary. It requires an
exact `HistoricalInputSnapshot`, constructs `PastRaceSnapshot`,
`TrackConditionsSnapshot`, `ImmutableRacePredictionInput`, and `InputSnapshotAudit`
directly, and returns the exact `SimulationRaceInput` after its existing validation.

Canonical entry and history ordering is explicit. The exact internal `race_entry_id`
keys every prediction mapping and is the semantic value carried by the legacy-named
`PastRaceSnapshot.horse_id`. Horse number, external identity, horse/jockey name, legacy
database mappings, and tuple accidents never participate in identity.

One checked Decimal-to-float helper owns target odds, past odds, weight, and signed
weight difference. It accepts normal deterministic binary-float approximation without
rounding, while finite output, field positivity, sign preservation, and nonzero
underflow checks fail closed. Exact snapshot Decimal values and `content_sha256` remain
the audit truth.

Every provenance item becomes one audit entry with unchanged metadata. Required
evidence reduces conservatively: observed time is the maximum observation; availability
is `None` if any required availability is unknown, otherwise the maximum availability.
The canonical output order is entry, odds, jockey, then indexed history or formal
absence for each entry, with track last. The audit source is the exact snapshot source
system and completeness is inherited only from the exact formal snapshot contract.
...
- `git diff --check`: pass;
- static scope, forbidden dependency, broad-catch, package-root, and public-surface
  checks: pass; and
- changed-file scope: exact four approved files.

C4f0, simulation validation, snapshot domains, prediction contracts, ValueEngine,
schema, and migrations are unchanged. The adapter owns no database, repository,
provider, HTTP, filesystem, current clock, legacy lookup, result/payout/settlement,
...
## Phase 4C-2d3b1i6d1d5f1c4g0 Implementation
...
Implemented the exact single-race historical prediction-to-persisted-bet-plan
composition. It accepts only one exact `HistoricalInputSnapshot`, exact
`SimulationRunContext`, exact `StrategyIdentity`, exact `BetStakeBudget`, and a
structural snapshot repository with `save_snapshot`. Boundary validation, exact
`RuleBasedBetStrategy` name binding, and exact dataset matching all occur before the
adapter, historical pipeline factory, or repository activity. Dataset mismatch raises
the frozen `SimulationValidationError` with the snapshot internal race ID and
`run_context.dataset_id` input identifier.

`ExactRaceEntrySelectionResolver` is a frozen/slotted module-local type with the exact
one-symbol public surface. It accepts only positive internal race-entry IDs, binds one
race ID and allowlist, and returns requested recommendation IDs unchanged in supplied
order. It performs no database, `RaceEntrySource`, horses-table, name, external-ID, or
legacy identity lookup.

The composition invokes c4f1 exactly once, creates one date-specific historical
pipeline exactly once using the exact caller-owned strategy-config object, then proves
the exact pipeline, config, strategy-config object identity, and `RuleBasedBetStrategy`
type before reuse of the unchanged `PersistedSimulationBetPlanService`. That service
continues to own the single pipeline run, allocation, plan building, validation, and
single snapshot save. The fixed-stake allocator receives only the exact strategy
allocation-policy object. A persisted empty-bets snapshot is the only planning-level
`NO_BET` outcome; input, strategy, validation, budget, selection, pipeline, and
repository failures propagate and never become an empty plan.

The two new modules own no direct SQLite/database read, race-entry source, horses table,
repository load, result/payout/settlement, simulator, HTTP, filesystem, clock, random,
or broad exception handling. Snapshot-repository I/O remains allowed only through the
existing `save_snapshot(...)` persistence contract; repository-internal idempotence and
integrity work never participates in recommendation identity transformation. C4f0,
c4f1, generic services, schemas, and migrations are unchanged. C4g1 and c4g2 were not
started.

Verification:

- dedicated: `19 passed, 32 subtests passed`;
- related: `182 passed, 65 subtests passed`;
- full suite: `2905 passed, 1955 subtests passed`; and
- static public-surface, forbidden-dependency, broad-catch, package-root, unchanged
...
## Phase 4C-2d3b1i6d1d5f1c4g1 Implementation
...
Review branch:
`review/4c-2d3b1i6d1d5f1c4g1-historical-multi-race-planning`.

Implemented the single c4g1 batch composition API. It accepts only an exact immutable
snapshot tuple, one shared exact run context and RuleBased strategy identity, a
defensively frozen race-ID budget mapping, and one structural snapshot repository. It
completes all static validation before any c4g0 call, including duplicate race rejection,
exact budget coverage, and deterministic canonical-order dataset preflight.

Each canonical `(scheduled_start_at, internal_race_id)` snapshot delegates exactly once
to unchanged c4g0 with the exact snapshot, shared context/strategy/repository, and exact
frozen budget object. C4g1 performs no direct save, pipeline/adapter/allocator/resolver
construction, database access, snapshot loading, result/payout read, settlement, or
simulation. A legitimate empty-bets c4g0 result remains in the complete returned tuple.

There is no batch transaction. A failure propagates unchanged, returns no partial tuple,
does not execute later races, and may leave a successful durable prefix from delegated
c4g0 calls. C4g1 does not retry, roll back, delete, compensate, load prior snapshots, or
assume generic repository idempotence. Determinism requires equivalent successful
repository start state and behavior; restart only guarantees no process-local,
clock/random/environment dependency, not replay across changed durable repository state.

Only the new batch module, its dedicated test, and these two phase docs changed. C4g0,
c4f0, c4f1, generic services, repository protocols, SQLite repository, schema, and
migrations remain unchanged; c4g2 remains unstarted.
...
## Phase 4C-2d3b1i6d1d5f1c4g2a Implementation
...
Implemented the new module-local `HistoricalPersistedRaceSettlementSource`. It remains
generic over the structural `SimulationBetSource`, validates its exact returned tuple,
and defensively freezes one mapping of aware settlement cutoffs keyed by positive exact
internal race IDs. A missing race cutoff fails before the bet source or either official
repository is called. An exact empty bet tuple returns empty
`PersistedRaceSettlementData` without result or payout reads; it does not claim persisted
c4g1 plan provenance.

For nonempty bets, the source reads the race result once and treats a result observed
after the cutoff as unavailable. It queries each distinct purchased bet type once in
first-occurrence order using the exact cutoff as `observed_at_lte` and
`require_complete=False`. Repository publications after the requested cutoff fail
closed. Missing and incomplete latest eligible publications are omitted with no second
query and no older-complete fallback. The source persists no publication identity,
writes nothing, owns no transaction, and uses no clock, database, HTTP, prediction, or
planning dependency.

Only the new source, its dedicated test, and the two phase docs changed. Existing generic
settlement source, persisted bet source and executor, repository protocols and concrete
SQLite repositories, c4f1/c4g0/c4g1, schemas, and migrations remain unchanged. C4g2b
remains unstarted, and real result/payout acquisition remains a separate phase.

Verification: dedicated `15 passed, 23 subtests passed`; related `205 passed, 61
...
## Phase 4C-2d3b1i6d1d5f1c4g2b Implementation
...
Implemented the single module-local `execute_historical_settlement_simulation` API. It
accepts only the exact historical snapshot tuple, exact run context and RuleBased
strategy identity, exact aware per-race cutoff mapping, and structural persisted-plan,
result, and payout read boundaries. Its deterministic 15-stage preflight completes
before any adapter or repository activity, including canonical ordering, duplicate
rejection, exact cutoff coverage, and canonical-order dataset mismatch selection.

Every c4f1 adapter call completes exactly once in canonical order before the composition
can load a plan or read an official fact. C4g2b then constructs exactly one
`PersistedSimulationBetSource`, cutoff-aware c4g2a source, unchanged persisted executor,
and unchanged `Simulator`, and returns the exact result of one `Simulator.run` call. The
valid empty batch follows the same composition and uses `Simulator.run(())`; no summary
is constructed manually.

The real integration tests prove the exact five-field persisted plan load, missing and
wrong-identity failure, persisted empty-plan `NO_BET` with no official reads,
after-cutoff result and incomplete-payout `UNSETTLED` behavior, after-cutoff publication
irrelevance, settled win and complete-publication loss arithmetic, mixed statuses,
settled-time/race-ID drawdown, deterministic caller-order behavior, and exact mid-batch
failure propagation. Component spies pin exact object wiring and construction/run counts.

The returned value is documented as an `AS_OF_SETTLEMENT_CUTOFF_SUMMARY`. Planned stake
for an intended race still `UNSETTLED` at its cutoff is excluded from settled investment
and the ROI denominator; such a partially settled summary is not claimed as guaranteed
final historical ROI. Existing Simulator metrics remain unchanged.

Only the new composition module, its dedicated test, and these two docs changed. C4f1,
c4g0, c4g1, c4g2a, the persisted bet source and executor, Simulator, repositories,
protocols, models, schema, migrations, and package root remain unchanged. C4g2c and real
official result/payout acquisition remain unstarted.
...
## Phase 4C-2d3b1i6d1d5f1c4h0 Trusted Evidence Acquisition
...
The approved baseline accessS URL was acquired through the unchanged formal
`JRAOfficialLiveResponseCaptureService` and a dedicated isolated migrated SQLite
archive outside the repository. The service used strict HTTPS, actual UTC clock
samples, full raw response bytes, immutable capture construction, and archive-before-
return ordering. Exact archive reload equalled the returned capture. No production
database was read for evidence identity or modified.

The trusted capture is
...
It was observed at `2026-08-26T11:38:28.113891+00:00`; its 94,570 exact cp932 bytes
have SHA-256
`f5daa967f05ae1ee0cfcbe8d4c0e59aa8a6b3ceef126ce9d8689fe10ffa8ed0e`.
The response is not backdated and supplies no historical availability claim before
that observation.

The URL parser and visible page agree on exact race
`jra:race:2025:06:04:03:04`: 2025-09-13, fourth Nakayama meeting, third day, race 4.
Exactly one formally headed result table contains 13 rows. Horse numbers 1 through 13
and numeric finish positions 1 through 13 each occur exactly once. Every horse number
deterministically yields the exact race-scoped external entry ID through
`build_jra_external_entry_id`; names and row positions were not used as identity.

The evidence is insufficient to authorize c4h0 persistence. The response contains no
independently established positive terminal marker such as `確定` or `成立`, so the past
date, populated result table, and payout area are not treated as proof of terminality.
The formal repository also contains no exact matching `HistoricalInputSnapshot`
crosswalk for this race, so mapping the 13 external entry IDs to exact internal
`race_entry_id` values and proving complete snapshot coverage is impossible. Normal
final is therefore captured structure but not yet supported. Dead heat,
scratch/exclusion, DQ/DNF, cancelled/void, and provisional representations were not
guessed or fabricated and remain fail closed.

No raw or derived fixture was committed. The repository currently uses compact derived
NAR parser fixtures rather than full official response archives, and this incomplete
evidence contract does not justify freezing a derived JRA grammar. The unmodified
original remains in the isolated archive, auditable by exact capture ID and SHA-256.
The existing past-race parser remains unchanged; only its strict cp932, accessS identity,
visible-header, result-table heading, and accessU-anchor contracts are candidates for
narrow later reuse.

The initial support envelope is empty: `COMPLETE`, `VOID`, `SCRATCH`, `DQ_DNF`, and
`DEAD_HEAT` are all `FAIL_CLOSED_NOT_YET_SUPPORTED`. Implementation readiness remains
`NO`, blocked specifically by positive terminal-finality evidence and an exact matching
snapshot-to-internal-entry crosswalk. No production, Python test, fixture, result
repository, schema, or migration changed. Pytest was not run. One authorized live
official request and trusted archive capture were performed. Stop for independent
evidence review; production implementation has not begun.

## Phase 4C-2d3b1i6d1d5f1c4h0 Normal-Final Evidence Contract Correction
...
Independent review identified a provider-semantic finality route omitted from the first
evidence assessment. The official JRA rule page
`https://www.jra.go.jp/keiba/rules/kakutei.html`, observed at
`2026-08-26T13:20:07.9999114+00:00`, states that race-result finalization and confirmed
finishing order precede payout announcement. The frozen semantic is
`PAYOUT_AMOUNTS_ARE_ANNOUNCED_AFTER_RACE_RESULT_FINALIZATION`. This documentation is
contract evidence only and is not a runtime dependency.

The original baseline capture was reloaded by exact capture ID from the isolated
archive without recapture. Capture ID, canonical URL, exact external race identity,
`2026-08-26T11:38:28.113891+00:00` observation, 94,570-byte length, and SHA-256
`f5daa967f05ae1ee0cfcbe8d4c0e59aa8a6b3ceef126ce9d8689fe10ffa8ed0e`
all match the approved evidence record.

The same exact accessS body has one formally recognized 13-row result table and exactly
one `#race_result .refund_area`. Its `.block_header` is `払戻金`; its single
`.refund_unit` contains eight bet-type items and twelve populated `.yen` values. These
are actual published official payout amounts, not an empty template, link, placeholder,
or estimate. The conjunction of official provider semantics and the same-race published
amounts proves
`NORMAL_FINAL_POSITIVE_TERMINALITY=PROVEN_BY_PROVIDER_POST_FINAL_PAYOUT_PUBLICATION`.
C4h0 may use that section only as a positive race-level finality fact. It does not parse
or persist payouts, and c4h1 remains unchanged and unstarted.

The initial result support envelope is now normal-final `COMPLETE` only. `VOID`,
`SCRATCH`, `DQ_DNF`, and `DEAD_HEAT` remain
`FAIL_CLOSED_NOT_YET_SUPPORTED`; provisional or absent positive terminality performs no
write. The ordinary capture is not generalized into any special-state grammar.

Snapshot readiness is separated correctly. C4h0 receives an exact
`HistoricalInputSnapshot` at runtime and can implement a generic complete crosswalk
validation boundary using exact JRA race identity and
`build_jra_external_entry_id`. A real matching snapshot is required at runtime but is
not required to implement that generic boundary. Tests may construct an exact snapshot
domain value without claiming historical observation. No causally valid real matching
snapshot exists for this baseline, so real end-to-end readiness remains `NO`.

C4h0 implementation readiness is `YES` for `NORMAL_FINAL_ONLY`, subject to independent
approval of this correction. Result `observed_at` remains the exact capture observation.
Because no exact provider finalization timestamp was found, `finalized_at` is the same
capture observation as the conservative first time this capture proves finality. No
production, Python test, fixture, schema, migration, repository, or production database
changed. No accessS recapture occurred. Stop before production implementation.

## Phase 4C-2d3b1i6d1d5f1c4h0 Implementation
...
Review branch:
`review/4c-2d3b1i6d1d5f1c4h0-jra-target-result`.

Implemented the module-local
`normalize_and_persist_jra_target_race_result` boundary and its four public error
classes. It accepts an exact nonempty capture ID, a structural immutable-capture archive,
an exact `HistoricalInputSnapshot`, and a structural `RaceResultRepository`. It performs
the public collaborator boundary validation before archive I/O, loads the exact capture
once, and never performs HTTP, live-capture work, archive writes, direct database access,
or a fallback lookup.

The implemented support is deliberately narrow:
...
The normalizer validates exact capture identity and `RACE_RESULT` page kind, exact JRA
source and accessS race identity, the visible date/venue/meeting/day/race header, one
formally headed whole-race result table, unique contiguous numeric horse numbers and
finish positions, and the approved same-race positive finality predicate: exactly one
`refund_area`, heading `払戻金`, one `refund_unit`, and positive official yen amounts.
It uses that payout publication only as terminal-result evidence: c4h0 does not parse
or persist payouts.

Every official horse number is resolved only through
`build_jra_external_entry_id` in the exact capture race and then through the exact
snapshot external-entry identity. Complete mutual result/snapshot coverage is required;
horse names, jockey names, row order, global horse-number lookup, cross-provider
matching, and prediction values are never identity sources. Only after all validation
succeeds does the boundary construct and save one `PersistedRaceResult` with
`RaceResultStatus.COMPLETE`, confirmed entries, exact capture observation for both
timestamps, and source equal to the exact capture ID. Archive and repository exceptions
propagate unchanged; failed validation never saves a result.

Only `scripts/simulation/jra_target_race_result_persistence.py`,
`tests/test_jra_target_race_result_persistence.py`, and these two documents changed.
Existing capture/identity/parser modules, repository protocols and SQLite code, c4g2a,
c4g2b, schemas, migrations, and package root remain unchanged. C4h1 is unstarted;
no final-ROI or matching real historical-snapshot claim is made.

Verification: dedicated `10 passed, 15 subtests passed`; required related
...
### C4j1 independent-review correction

Independent review rejected prior candidate
`5f80717c9fde71a5357d0e06682e72ade9c15712` only for two release-documentation
corrections. Runtime, architecture, schema, manifest, claim boundary, scope, and CI
were not rejected. The prior candidate remains unchanged and is not formal ancestry.
This replacement is built directly on exact formal base
`c6c2f17934c2274d5bc720c60e40e52bd0c3ee58` on
`review/4c-2d3b1i6d1d5f1c4j1-ver0.8-release-docs-correction`.

The corrected CHANGELOG retains the reviewed Ver0.8.0 (Unreleased) capability,
reproducibility, and limitation sections above the complete published historical
entries. The formal-base content from `Ver0.7 Race Engine` through the final Ver0.5
separator is preserved exactly, including the Ver0.7 release/commit record, Added,
Data Flow and Database sections; Ver0.6 Added and supported-racecourse record; and
Ver0.5 Added record. Only the obsolete active Future Versions tail is replaced by a
notice that future unapproved work lives under Post-Ver0.8 and the old Horse/Result/AI
sequence is not the current version contract.

The release notes retain all approved content except their Release State wording.
Branch-specific `At this C4j1 review-candidate stage` wording is removed. The durable
replacement states that release-candidate documentation has been prepared and
independently reviewed, master integration is pending, the tag and GitHub Release do
not exist, and final integration/tag/date/publication still require authorization.

The six unaffected reviewed files were materialized from the prior candidate and their
final blobs remain exactly:

```text
README.md
8834d21518c5be1562570fcf05d8a34450ebac9b

docs/ROADMAP.md
8f601b9b0e58a7cb81d22ae1b34682e2ccb9c06e

docs/ARCHITECTURE.md
1eb09d0d8fed7b50d35c43a14508aa5ac3f16183

docs/VER0.8_DESIGN.md
69d9cb1b6ad42d1ebb353e7c06beef094d11dba7

docs/HISTORICAL_REPLAY.md
5cf35feac9b09a5f5196dbec3ef552dd43f59609

examples/historical_replay_request.v1.example.json
f8e9f68b95486b684506202cad00ef1b9e00348a
```

The unchanged public loader accepts the example manifest. Local correction
verification passed:

```text
MANIFEST_PUBLIC_LOADER_VALIDATION:
PASS
...
PASS_EXACT_FORMAL_HISTORY_BLOCK
...
Production, tests, fixtures, schemas, migrations, workflows, package-version sources,
...
## POST_V0_8_DAILY_REPLAY_1 Final Commit and Push

ChatGPT's final independent review verdict was `APPROVED_FOR_COMMIT`. The exact two
approved design documents were staged by name and committed with message
`docs: design post-v0.8 daily replay orchestration`.

```text
local commit:  dca745c3f8e076ad8bdc9ae4cd320d0a6df7a0ad
remote branch: dca745c3f8e076ad8bdc9ae4cd320d0a6df7a0ad
branch:        feature/post-v0.8-daily-replay
```

The normal push succeeded, `git fetch origin` completed, local and remote SHAs matched
exactly, and the working tree and diff check were clean before Phase 2 preparation.
No production, test, migration, CLI, schema, database, archive, tag, or release-history
file was committed.

## PREPARE_PHASE POST_V0_8_DAILY_REPLAY_2

Status: `DRAFT_FOR_REVIEW` as a `DESIGN_ONLY` phase.

Phase name: `Historical Daily Target Discovery / Completeness Design`.
Base commit is the exact pushed Phase 1 commit
`dca745c3f8e076ad8bdc9ae4cd320d0a6df7a0ad` on
`feature/post-v0.8-daily-replay`.

The preparation read the working agreement, Ver0.8 simulator design, prior report, Git
history/state, and investigated legacy race storage, v010 external identity mapping,
JRA target navigation/card discovery and tests, JRA historical/capture domains, NAR
target and prior-race sources and tests, capture archive schemas/repositories, and the
legacy current NAR fetch/parser path. The tracked main DB was inspected only through a
read-only SQLite URI; it was not modified.

The central finding is that the baseline has no formal immutable provider-day source
which proves complete meeting/venue coverage for either JRA or NAR. The legacy `races`
table is nullable and has no day-completeness evidence or race uniqueness constraint.
`historical_input_external_races` maps known identities but has no coverage proof.
Individual snapshots, cards, results, payouts, and archive rows likewise cannot define
the denominator.

JRA's exact identity, request locator, v4 target-race-selection capture, and strict
race-list parser are reusable meeting-level fragments. They begin with a known external
race/meeting, while the current root/meeting responses used by live acquisition are not
archived and cannot prove an arbitrary historical day's complete meeting set. NAR's
formal capture vocabulary contains individual DebaTable, RaceMarkTable, and horse
history pages only. The legacy TopTodayRaceListMini parser is current/live, silently
skips malformed material, can return empty on missing structure, and has no formal
capture identity, digest, time, or coverage contract; it cannot be historical evidence.

The draft therefore defines a new provider-specific immutable
`HistoricalDailyTargetEvidenceBundle` as the formal input and an audited
`DailyHistoricalReplayTargetSet` as its content-addressed canonical projection. Shared
logic validates exact closed provider coverage, partition coverage, target identity,
proven zero, conflicts, and deterministic ordering. Provider adapters own only formal
historical source validation and native identity/status projection. Existing JRA
formal parsing must be reused rather than duplicated.

The minimal target-set proposal freezes exact date and provider scope, canonical target
races with exact provider identity, `scheduled_start_at`, race-level disposition,
completeness evidence identity/source/digest/time/coverage, and a deterministic
target-set identity. Internal SQLite IDs are intentionally excluded and remain Phase B
reconciliation work. A cancelled/postponed/abandoned race remains in the denominator;
it is not confused with a race that never existed.

An empty target tuple is accepted only when every provider in scope supplies positive
formal zero-day evidence. Any missing provider/partition proof, duplicate, contradiction,
unknown status, or invalid evidence returns no set and fails the whole request as
`TARGET_DISCOVERY_INCOMPLETE`; no arbitrary tie-break or partial denominator is allowed.

Acquisition is separated from no-network replay. Acquisition must retain honest
observation time and exact immutable evidence without backdating. Replay exact-loads
the frozen set and never fetches or falls back. Exact official historical daily sources,
postponement semantics, availability-time support, and durable storage choice remain
explicit review questions. No source/repository/schema/migration choice is guessed
before those sources are proven.

Only `docs/CURRENT_PHASE.md` and `docs/LATEST_CODEX_REPORT.md` were changed. No code,
tests, migrations, schemas, CLI, databases, archives, fixtures, stage, commit, push, or
next phase is authorized by this PREPARE activity.

## POST_V0_8_DAILY_REPLAY_2 Independent Design Review Corrections

Status: `APPROVED_FOR_CODEX` as a `DESIGN_ONLY` phase.

ChatGPT independently reviewed the Phase 2 draft with verdict
`APPROVE_WITH_CORRECTIONS`. Only the two approved design documents were corrected;
production code, tests, migrations, schemas, CLI, databases, archives, fixtures,
provider evidence, stage, commit, push, and later phases remain untouched and
unauthorized.

The scheduled-start contract now distinguishes normal replay-candidate targets from
non-run/exceptional denominator members. A normal candidate requires an exact aware
`scheduled_start_at`. An exceptional target retains an exact original scheduled start
only when formal official evidence supplies it and otherwise uses `None`; no display
text or expected timetable may fill it. The Phase 1
`selection_upper_bound = audited_target.scheduled_start_at` rule applies only to later
replay candidates with a formally present exact start. A target with no start remains
in the denominator and is ineligible for `LATEST_CAUSAL_IN_DATASET`.

Canonical target-set order no longer depends on nullable time. It is exactly
`(organization, source_system, external_race_id)`. Provider official order may be
retained as evidence but cannot determine canonical identity. Display place, SQLite
identity, insertion order, current clock, and nullable start are excluded.

The earlier wording which appeared to freeze a provider-neutral disposition enum was
withdrawn. Phase 2 freezes only semantic requirements: official membership is never
erased; provider-native status/disposition evidence remains auditable; unknown or
contradictory status does not fall back to normal; and denominator membership remains
separate from later replay classification. Exact JRA/NAR status semantics and any
closed normalized enum require real-source qualification and another ChatGPT review.

Completeness-evidence `observed_at` is now explicitly the honest time KeibaOS acquired
or observed the historical official evidence and may be later than the historical
date. It must not be backdated. It is audit/orchestration metadata, not prediction
`information_cutoff`, snapshot upper bound, settlement cutoff, provider availability,
or scheduled start, and it must not flow into prediction or strategy. Formal
`provider_available_at` is retained only when the provider source itself supplies it.

Concrete but unapproved candidates are now recorded. JRA qualification will examine
the official historical JRADB `accessD`/`accessS` navigation/race-selection family and
reuse the repository's strict navigation/identity grammar, while recognizing that
saved v4 fragments alone do not prove provider-day completeness. NAR qualification
will examine date-qualified official `TodayRaceInfo` venue `RaceList` pages using
`k_babaCode` and `k_raceDate`, including possible same-day venue navigation. Legacy
`TopTodayRaceListMini`/`NARParser` remains unsuitable as-is. Neither candidate is
approved until historical request identity, all partitions/races, zero-day and non-run
semantics, conflicts, charset/content, exact bytes/digest, honest times, historical
availability, deterministic normalization, and no-skip/no-backdating behavior are
proven.

`HistoricalDailyTargetEvidenceBundle` is clarified as one shared provider-neutral
immutable normalized contract containing exactly one provider identity. Provider
adapters produce that common bundle; separate provider bundle classes require a proven
semantic need. Raw official evidence remains primary, while bundle and target set are
traceable immutable audited projections retaining exact source identity, capture or
reference identity, digest, and provenance.

The target set now has one formal deterministic identity field: `content_sha256`.
`target_set_id / content_sha256` was removed. A future artifact phase may derive
`daily-target-set-v1:<content_sha256>` without altering digest semantics, but no durable
storage is designed here.

The recommended next phase is recorded but not started:

```text
POST_V0_8_DAILY_REPLAY_3
Historical Daily Official Source Qualification
RESEARCH_AND_DESIGN_ONLY
```

It will qualify the candidate JRA/NAR official historical sources before any target
discovery production implementation. Evidence Resolver implementation must not begin
before that qualification is formally reviewed and approved.

## POST_V0_8_DAILY_REPLAY_2 Final Independent Review Approval

ChatGPT completed the final independent review with verdict `APPROVED_FOR_COMMIT`.
Phase `POST_V0_8_DAILY_REPLAY_2` remains `DESIGN_ONLY`; the approved state changes no
production code, tests, migrations, schemas, databases, archives, CLI, release tag, or
Ver0.8 contract.

The final approval confirms these contracts:

- the daily denominator is never inferred from legacy races, snapshots, or result
  captures and instead requires positive official provider-day completeness evidence;
- one shared provider-neutral `HistoricalDailyTargetEvidenceBundle` is the normalized
  immutable contract, while raw official material remains the primary source evidence;
- `DailyHistoricalReplayTargetSet` is an audited immutable projection which fails
  closed as `TARGET_DISCOVERY_INCOMPLETE` unless complete coverage or an explicit
  proven-zero day is established;
- `scheduled_start_at: datetime | None` requires an exact aware value for a normal
  replay candidate, preserves exceptional targets with `None` in the denominator, and
  never participates in canonical target ordering;
- canonical target ordering is exactly
  `(organization, source_system, external_race_id)`;
- provider-native disposition evidence is preserved without prematurely freezing a
  provider-neutral closed enum;
- completeness `observed_at` is honest acquisition time, is never backdated, and is
  never prediction, snapshot-selection, or settlement causal time;
- `provider_available_at` exists only when the official source formally supplies it;
- `content_sha256` is the sole deterministic target-set content identity;
- acquisition/preparation remains separate from no-network replay;
- the JRA/NAR source candidates remain unqualified pending Phase 3; and
- the v0.8.0 release contract and history remain unchanged.

Only `docs/CURRENT_PHASE.md` and `docs/LATEST_CODEX_REPORT.md` are authorized for the
Phase 2 final design commit.

## POST_V0_8_DAILY_REPLAY_2 Final Commit and Push

The approved Phase 2 design was committed as
`873ab8eb854716c93886ed689cee93c1ef8bfd23` with message
`docs: design historical daily target completeness` and pushed normally to
`origin/feature/post-v0.8-daily-replay`. After `git fetch origin`, local HEAD and the
remote branch both resolved to that exact SHA. The post-push working tree and diff check
were clean, so the explicitly authorized Phase 3 PREPARE was allowed to begin.

## PREPARE_PHASE POST_V0_8_DAILY_REPLAY_3

Phase name: `Historical Daily Official Source Qualification`.

Status: `DRAFT_FOR_REVIEW` as `RESEARCH_AND_DESIGN_ONLY`.

Base commit: `873ab8eb854716c93886ed689cee93c1ef8bfd23` on
`feature/post-v0.8-daily-replay`.

The repository's formal JRA/NAR identity, target-card, supplied-response, capture,
repository exact-load, historical-input, and related tests were inspected before public
research. Read-only public research was then limited to official `jra.go.jp` and
`keiba.go.jp` material. Responses were inspected only as research material and were not
saved to repository databases, logs, fixtures, or provider archives and were not
backdated as historical captures.

### JRA research result

The official accessS past-results search is materially stronger than an unknown known-
race bootstrap. Posting the official past-search locator exposes exact year/month
selection and an official opaque-tail map; the exact month request returns meeting/day
CNAMEs with date, venue, meeting, and day identity; each meeting request returns race
selection; direct accessS result pages retain meeting and race navigation. Exact old
month responses were observed for January 2020 and January 2015. The inspected bytes
declare Shift_JIS and decode strictly as CP932, so exact-byte capture and SHA-256 are
technically possible.

The 2024-09-01 official static daily program listed Niigata, Chukyo, and Sapporo with
twelve planned races each, but explicitly describes itself as planned information which
can change or be cancelled/postponed. On 2019-10-12 the official static page records the
Tokyo cancellation and substitute date 2019-10-15 while accessS results retain Kyoto on
the 12th and Tokyo on the 15th. This proves a concrete cancellation case but not the
general identity/membership mapping for postponed targets. No positive zero-day source
was established, and no inspected contract guarantees that the monthly result list is
an exhaustive provider-day envelope including all cancelled/non-run meetings.

JRA Phase 3 outcome: `UNPROVEN`.

### NAR research result

Historical date-qualified `TodayRaceInfo/RaceList` pages successfully bound an exact
date and venue, directly enumerated venue race rows, and exposed same-day venue
navigation. Multi-venue examples were observed for 2025-05-05 and 2024-09-01, and the
Saga page for 2020-01-03 remained available. Exact response bytes use UTF-8 and are
technically digestible.

Two concrete official whole-race cancellation cases proved that the historical
RaceList can preserve target membership: Funabashi 2025-08-30 retained all twelve rows
and announced cancellation of races 10 through 12; Kochi 2026-04-04 retained all twelve
rows and announced cancellation from race 4 with no substitute meeting. Official result
books independently retain the affected race identities as `競走取止め`.

The provider-day boundary nevertheless remains unproven. No inspected contract states
that same-day venue navigation is exhaustive, a no-racing request returned only a
generic error rather than positive zero evidence, and exact request identity remains
ambiguous across literal versus percent-encoded date slashes, host variants, and zero-
padded versus canonical decimal baba codes. Postponement/abandonment mappings also remain
unqualified.

NAR Phase 3 outcome: `UNPROVEN`.

### Qualification decision

Both providers have plausible unapproved composite candidates:

```text
JRA accessS historical month envelope
  -> exact meeting/day race-selection fragments
  -> official static cancellation/status material when needed

NAR historical same-day venue navigation envelope
  -> exact date-qualified RaceList fragment per venue
  -> official provider-native cancellation/status material when needed
```

Neither composite is frozen. Provider-day envelope completeness, positive zero, and
general non-run semantics remain unproven, so the approved Phase 2 behavior stays
`TARGET_DISCOVERY_INCOMPLETE`. No Phase 2 contract was changed: the shared provider-
neutral bundle, raw-primary-evidence rule, nullable scheduled start, native disposition
evidence, honest acquisition observation, deterministic ordering, and
`content_sha256` remain intact.

Only `docs/CURRENT_PHASE.md` and `docs/LATEST_CODEX_REPORT.md` were changed during this
PREPARE. No production code, tests, migrations, schemas, database, archives, CLI,
fixtures, stage, commit, push, or next phase was authorized or performed.

## POST_V0_8_DAILY_REPLAY_3 Independent Review Corrections

ChatGPT returned `CHANGES_REQUIRED` for the initial Phase 3 research draft and requested
deeper qualification of provider schedule/envelope candidates. The phase remains
`DRAFT_FOR_REVIEW` and `RESEARCH_AND_DESIGN_ONLY`; this activity authorizes no production
implementation, tests, migrations, schemas, database, archives, CLI, stage, commit,
push, or Phase 4.

### NAR MonthlyConveneInfo research

The official historical `MonthlyConveneInfoTop?k_year=YYYY&k_month=M` family was added
as the strongest provider-day envelope candidate. It presents historical date columns
and all displayed racecourse rows, links marked cells to venue/date race information,
defines ordinary/night/dirt-grade marks and `△` for substitute dates, and warns that
disasters can alter schedules. The official NAR FAQ directs historical result users to
this monthly schedule and states that all conducted race results since 1998 are
viewable.

For the tested ordinary dates, monthly marked venue sets exactly matched RaceList
same-day navigation: six venues on 2025-05-05, four on 2024-09-01, and three on
2025-08-30. This agreement was recorded as research, not generalized into an exhaustive
contract.

Adjustment cases exposed the planned/actual boundary. The December 2017 table marks
Kanazawa December 19 with `△` after the December 17 cancellation, but the original date
cell is blank and does not retain exact original race identities. On 2025-12-26 the
monthly table still marks Kanazawa `●` even though the historical RaceList states that
the whole meeting was cancelled with no substitute. The monthly mark therefore proves
schedule/meeting membership, not that races were conducted; native RaceList/status
evidence remains necessary.

Every displayed venue row was blank on 2020-03-09. The official legend does not define
blank as an audited actual-zero assertion, and the FAQ's conducted-result statement does
not establish immutable post-adjustment blank semantics. `PROVEN_ZERO_SUPPORTED` and
`PROVIDER_DAY_PARTITION_COMPLETE` therefore remain `UNPROVEN`. The earlier 2025-01-01
zero attempt was corrected: MonthlyConveneInfo shows Kawasaki, Nagoya, and Kochi racing,
so a generic error from one arbitrary venue request was never zero evidence.

The strongest NAR composite candidate is now:

```text
MonthlyConveneInfo historical year/month envelope
  -> one exact date-qualified RaceList per marked venue
  -> exact provider-native cancellation/substitute evidence when needed
```

It remains unapproved because blank-cell zero semantics, exhaustive actual-versus-
planned coverage, original identity on moved meetings, and canonical RaceList request
identity are not proven.

### JRA annual schedule and adjustment research

The official historical annual Racing Schedule, year-specific program pages,
`開催日割表`, schedule decision documents, racing calendar, and schedule/cancellation
FAQ were added to the source candidates. The 2024 materials provide an exact annual
planned schedule and state that the year's first meetings were Nakayama and Kyoto on
January 6. The daily program pages themselves state that they are pre-announced plans
subject to cancellations, postponements, and changes, so an unlisted date such as
2024-01-02 is a planned-zero candidate but not yet a qualified post-adjustment actual
zero.

The 2019-10-12 static page retains the cancelled Tokyo meeting and its October 15
substitute while preserving Kyoto on October 12; the October 15 page contains the Tokyo
replacement program, accessS exposes Kyoto results on the 12th and Tokyo results on the
15th, and the JRA FAQ explains that some later conduct may occur without new entry when
formal conditions hold. These sources agree for the case, but no inspected official
index proved exhaustive schedule adjustments or exact original-to-replacement race
identity across every cancellation mode.

Two composites were compared. The preferred unapproved direction begins with the
annual/period planned schedule, then requires an exhaustive exact adjustment source,
then exact accessS meeting/race fragments. An accessS-first composite cannot recover a
fully cancelled original denominator from absence. JRA remains `UNPROVEN`.

### Contract and matrix corrections

Denominator semantics now separately name races conducted on target date, races
scheduled but cancelled there, substitute-date conduct, and original versus replacement
identity. Race identities may not be synthesized from a meeting-level schedule.

Every matrix property now has a one-line definition. Historical-date addressability is
explicitly separate from completeness and zero. Availability was split into
`OBSERVED_AVAILABLE_TO_TESTED_FLOOR` and the new
`REQUIRED_RETENTION_HORIZON_SUPPORTED`; the former is qualified only for the observed
JRA January 2015 and NAR January 2020 RaceList floors, while required retention remains
`UNPROVEN` for both providers. `FORMAL_PROVIDER_AVAILABLE_AT_HANDLING` replaces the
ambiguous provider-time label: the tested contracts yield exact `None`, forbid a
substitute, and make no permanent claim about other source families.

Research-observation wording now states exactly that Phase 3 responses were honestly
observed but were not formally captured, qualified, or persisted as bundle evidence;
their timestamps are not formal completeness-evidence `observed_at` values. Any future
authorized acquisition must record its own exact retrieval time and must not backdate.

The formal outcomes remain:
...
Accordingly, future attempted discovery remains fail-closed as
`TARGET_DISCOVERY_INCOMPLETE`. Phase 2 contracts are unchanged, and no provider source,
bundle, target set, archive, or implementation has been approved or created.

## POST_V0_8_DAILY_REPLAY_3 Final Independent Review Approval

ChatGPT completed the final independent review and approved Phase 3 for commit. The
approved formal research conclusions are `JRA: UNPROVEN` and `NAR: UNPROVEN`; this is
the intentional fail-closed research result, not a phase failure.

The review formally approved all of the following conclusions:

- a planned schedule is not automatically the actual race denominator;
- positive zero-day semantics remain unproven;
- cancellation and substitute-date original race identity remain unproven;
- universal provider-day partition completeness remains unproven;
- NAR `MonthlyConveneInfo` is the strongest provider-day envelope candidate;
- NAR date-qualified `RaceList` is the strongest venue-day race-list fragment candidate;
- the JRA annual/period schedule is the strongest planned-envelope candidate;
- JRA `accessS` is the strongest actual historical meeting/race fragment candidate;
- composite official evidence is permitted conceptually but is not universally qualified;
- Phase 3 research responses are not formal historical replay evidence;
- any future formal acquisition must record an honest `observed_at` and must never
  backdate it;
- every approved Phase 2 contract remains unchanged; and
- `TARGET_DISCOVERY_INCOMPLETE` remains the correct future behavior for unqualified
  provider dates.
