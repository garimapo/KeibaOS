...
## Phase 4C-2d3b1i6c1d3b2b Trusted NAR Response Capture/Archive Preparation

Status: `DRAFT_FOR_REVIEW` on `review/4c-2d3b1i6c1d3b2b-prepare`, directly based on formal
`4af5a7ba4f18769f365ac2c934bcfd0ffcf38818`. This preparation changes only the phase documents; it does not add an
archive, HTTP capture, v013, fixture, test, or orchestration.

The frozen model is `OPERATIONALLY_TRUSTED_LOCAL_CAPTURE`: KeibaOS itself will receive exact parser-input entity bytes,
timestamp `observed_at` after complete receipt, retain those bytes, and recompute SHA-256 at archive save/load. The
claim is deliberately not cryptographic external timestamp authority. Retroactive current-live-page backfill is
impossible to trust; before deployment historical live-site-only replay is valid no-data, not a timestamp/fallback
problem. `available_at` remains None, c1a/c1b/d3b2a/snapshot contracts remain unchanged, and reconstruction joins by
the existing `(canonical_source_url, response_sha256, observed_at)` evidence tuple.

The approved three-kind URL vocabulary is DebaTable, HorseMarkInfo, and RaceMarkTable. The legacy NARProvider text,
apparent-encoding, and logs path is explicitly not archive evidence. Official read-only probes on 2026-08-10 observed
direct `www` examples as UTF-8 HTTP 200 with no Content-Encoding and 297,718 / 83,949 / 96,614 byte bodies. The
observed `www2` HorseMarkInfo 301 is only a temporal transport probe; the permanent unit-test rule is any 3xx fails
closed under disabled redirects and creates no capture.

The corrected d3b2b1 public surface is exactly page kind, immutable capture, archive protocol, capture base/
validation/unsupported/missing errors, and strict NAR URL canonicalization. SQLite archive caller/conflict/corruption
paths reuse `RepositoryValidationError`, `RepositoryConflictError`, and `RepositoryDataIntegrityError`; missing exact
evidence remains the capture-domain missing error. d3b2b2 exposes only live capture service, transport error, and a
production factory that owns private requests transport and real UTC clock. Future d3b2c may not import private
transport or implement HTTP.

v013 is frozen as additive `v013_nar_official_response_capture_schema`, with content-addressed bodies and immutable
capture observations. Its one named UNIQUE lookup index is
`ux_nar_official_response_captures_evidence(canonical_source_url, response_sha256, observed_at_utc)`; no redundant
ordinary lookup index is permitted. v013 is transaction-neutral and the migration runner owns transactions. The
complete DDL, Content-Length/body matching, timestamp format/order, and stored-at limitation are in CURRENT_PHASE.

The archive remains in the same KeibaOS SQLite DB. The illustrative collection model is approximately 11.5 GiB/year
for 10,000 target races and 17.2 GiB/year for 15,000 before overhead, changed bodies, repeated observations, or extra
historical result coverage. Therefore archive-aware backup/WAL/disk operations and no automatic retention deletion are
explicit operational requirements. A separate archive DB needs a future dedicated export/migration phase.

Repository-wide migration-registry scan is complete. The exact existing tests requiring v013 expectation updates are
`tests/test_historical_input_snapshot_migration.py`, `tests/test_simulation_bet_plan_migration.py`,
`tests/test_simulation_migrations.py`, and `tests/test_sqlite_persisted_simulation_application.py`; no additional
hard-coded final-v012 test was found.

## Phase 4C-2d3b1i6c1d3b2b Final Separate-Capture-Database Architecture Correction

The final d3b2b PREPARE correction keeps all approved local-capture, strict URL/page, byte-identity, public API, transport, timestamp, replay-causality, and fail-closed archive conclusions. It changes only storage topology.

`DATABASE_LOCATION = SEPARATE_NAR_CAPTURE_DATABASE`. The illustrative archive projection is about 11.5 GiB/year for 10,000 target races and 17.2 GiB/year for 15,000 target races before indexes, WAL, backups, changed bodies, repeated observations, or added historic coverage. The blob archive has a different WAL/checkpoint, backup, vacuum, and retention lifecycle from the main simulation database. Separation prevents archive growth and maintenance from burdening main KeibaOS storage, and confines a failure in either database from corrupting the other. Automatic retention delete remains forbidden.

There is no cross-database FK, new c1a/snapshot field, or distributed transaction. `CROSS_DATABASE_LINKAGE` remains the existing evidence tuple `(canonical_source_url, response_sha256, observed_at)`. Capture persists before later normalization; an unused capture after normalizer failure is safe, while failed persistence supplies no trusted operational evidence. `CROSS_DATABASE_ATOMIC_TRANSACTION = NOT_REQUIRED`; repository code must never `ATTACH` a DB. The repository receives an injected capture SQLite connection and knows no main database path/tables. A later d3b2c composition phase owns distinct `capture_database_path`, separate from main `database_path`.

The main/global v013 proposal is withdrawn. `GLOBAL_MIGRATION_REQUIRED = NO`; formal global sequence remains 8, 9, 10, 11, 12. `scripts/migrations/runner.py`, global migration files, and the four previously identified global migration-expectation tests remain unchanged. `GLOBAL_MIGRATION_REGISTRY_SCAN = COMPLETE` and `GLOBAL_MIGRATION_REGISTRY_AFFECTED_TESTS = NONE`.

Capture schema migration is dedicated: `CAPTURE_SCHEMA_MIGRATION_REQUIRED = YES`, version `1`, name `v001_nar_official_response_capture_schema`, in `scripts/simulation/nar_official_response_capture_migration.py` and `scripts/simulation/nar_official_response_capture_migration_runner.py`. The dedicated runner owns only capture DB: it validates `nar_official_response_capture_schema_migrations`, rejects an active transaction and invalid registry states, enables/verifies foreign keys, applies pending migrations in ascending order, and atomically inserts registry state with `BEGIN IMMEDIATE`; migration `apply(connection)` remains transaction-neutral. It neither imports global `MIGRATIONS` nor creates/inspects simulation tables. v001 creates only the frozen body table, capture table, and `ux_nar_official_response_captures_evidence` unique evidence lookup index.

Operations back up `main KeibaOS SQLite db` and `NAR trusted capture SQLite db`, preferably main then capture. Backup skew has no web fallback; unavailable exact capture lookups fail closed. The d3b2b1 future scope is exactly nine files: capture domain, SQLite repository, dedicated migration module/runner, three dedicated test files, and phase/report docs. d3b2b2 remains four files only: live-capture module/test plus docs. No code was implemented, no capture was made, and status remains `DRAFT_FOR_REVIEW` pending architecture re-review.
