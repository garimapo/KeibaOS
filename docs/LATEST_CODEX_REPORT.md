...
## Phase 4C-2d3b1i6d1d5f1c4h0 Identity Validation Correction
...
Correction branch:
`review/4c-2d3b1i6d1d5f1c4h0-jra-target-result-correction`.

Materialized the four exact previous-review blobs onto the exact formal base, then
applied only the approved identity and dedicated-test corrections. The normal-final
support envelope, exact capture load, positive payout-publication finality predicate,
race-local snapshot crosswalk, no-payout-normalization boundary, and one-save-after-
validation contract remain unchanged.

The corrected normalizer now parses the calendar `YYYYMMDD` from the complete canonical
accessS URL with a strict local grammar and requires:

```text
ACCESS_S_CNAME_CALENDAR_DATE:
MUST_EQUAL_VISIBLE_HEADER_DATE_AND_SNAPSHOT_TARGET_RACE_DATE

VISIBLE_RACE_NUMBER:
EXACT_FULL_VALUE_1R_TO_12R
```

A same-formal-race accessS URL carrying a different valid calendar date now fails before
result construction or save. Malformed and impossible local CNAME dates fail closed.
The visible race number uses whole-value matching, rejecting `14R`, prefixes, suffixes,
embedded values, and multiple race values.

The dedicated suite now directly covers a real exact
`HORSE_PROFILE_HISTORY` capture, all visible header fields, duplicate/missing result
horse-number and place cells, extra/missing result membership, incoherent external-entry
identity, wrong external race, duplicate mapped internal entry identity, all required
finality-container failures, and the complete unsupported special-finish freeze. Every
failure asserts zero result saves. Horse name, jockey, row order, and global horse-number
fallback remain absent.

Only `scripts/simulation/jra_target_race_result_persistence.py`,
`tests/test_jra_target_race_result_persistence.py`, and the two phase documents changed.
`jra_official_identity.py`, capture, past-race parser, repositories, SQLite, schemas,
migrations, c4g2a, c4g2b, and package root remain unchanged. C4h1 was not started.

Verification: dedicated `16 passed, 48 subtests passed`; required related `141 passed,
254 subtests passed`; full suite `2980 passed, 2074 subtests passed`; static scope and
Git diff checks pass. No live HTTP or trusted capture was performed. Stop for independent
review; formal integration is not claimed.
