...
Implement the smallest fail-closed V3c boundary for immutable validated historical source records and deterministic
source IDs. This phase creates only the approved source-record domain and its dedicated tests; it does not
implement a provider, raw store, normalizer, snapshot builder, migration, or repository change. The nine V3a
domain values and protocols, the V3b SQLite schema, and the V3d SQLite repository save/load behavior are frozen.
...
## Exact c1a Allowed Files and Test Matrix
...
All production and tests outside the two Allowed Files, migrations/schema, provider/parser/fetch code, README,
package exports, database/logs, the original workspace, and Git changes not explicitly authorized by a later
commit instruction are forbidden.
...
Implementation is complete and Phase 4C-2d3b1i6c1a is `READY_FOR_REVIEW`, awaiting ChatGPT implementation
review and explicit commit approval. Do not stage, commit, push, or begin c1b/c1c/c1d without separate explicit
authorization.
