...
The formal structural `PastRaceInput` and immutable `PastRaceSnapshot` are now
marginless. Legacy mutable `PastRace.margin` and the schema-v1 persisted request field
remain unchanged and validated, but immutable conversion intentionally discards the
value. `AbilityEngine` reads only finish, popularity, and class and applies the exact
normalized 9:3:5 formula and retained-field eligibility.

`build_historical_prediction_pipeline(...)` validates exact `date` and exact
`StrategyConfig` before collaborator construction, injects the same official target
race date into Ability, Jockey, and Track, and explicitly constructs every normal
pipeline collaborator without sampling the current clock.

Focused tests passed with `44 passed, 339 subtests passed`; related regressions passed
with `55 passed, 149 subtests passed`; the full suite passed with
`2871 passed, 1921 subtests passed`. Static scope, forbidden-margin, c4f1-absence,
schema/migration-absence, AST ownership, and `git diff --check` checks passed.

No live HTTP or real trusted capture was performed. C4f1 remains unimplemented.
...
All other production and test files, including legacy mutable models, Jockey, Track,
Pace, Value, persisted-request production, historical snapshot/source/replay/persistence,
c4d, c4e, c4f1, schema/migration, CLI, live-capture, NAR, betting, settlement, and
package-root files.

## Required Implementation Checks

```text
pytest -q tests/test_ability_engine.py tests/test_prediction_input_contracts.py tests/test_prediction_pipeline.py tests/test_persisted_simulation_race_inputs.py
pytest -q tests/test_jockey_engine.py tests/test_track_engine.py tests/test_persisted_simulation_application_inputs.py tests/test_persisted_simulation_request_document.py tests/test_simulation_validation.py
pytest -q
git diff --check
git status --short
changed-file scope == the ten allowed files
AbilityEngine forbidden-margin search == no matches
c4f1 module == absent
schema/migration diff == empty
```

## Stop Condition

Commit and push the single c4f0 implementation review commit, then stop for independent
