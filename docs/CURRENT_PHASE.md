...
`4C-2d3b1i6d1d5f1c4g2` — Historical settlement and simulation summary composition.
...
The cutoff mapping is validated and defensively copied once at construction. Every key
...
The source must defensively reject a repository response with `observed_at` after the
...
No production or tests are implemented. Pytest, live HTTP, and trusted capture are not
performed.

```text
IMPLEMENTATION_READY:
YES_AFTER_INDEPENDENT_APPROVAL_FOR_C4G2A
