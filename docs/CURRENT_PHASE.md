...
All files outside the allowed set, including existing production and test files, schemas,
migrations, package roots, databases, and logs.

## Verification

Completed verification:

- dedicated: `13 passed, 20 subtests passed`;
- related: `50 passed, 67 subtests passed`;
- full suite: `2925 passed, 1975 subtests passed`; and
- changed-file scope and static ownership checks are covered by the dedicated suite.

`git diff --check` is required before review commit. No live HTTP or trusted capture is
allowed.

## Stop Condition

After one implementation review commit is pushed, stop for independent review. Do not
begin c4g2 or modify the formal branch.
