...
Implementation correction review branch:
...
## Identity Validation Correction
...
The normal-final-only architecture is unchanged. The corrected implementation now
extracts the exact `YYYYMMDD` from the already-validated canonical accessS CNAME using
a strict local whole-URL grammar. It requires the three independently supplied dates
to agree before result construction or persistence:
...
The visible header race number is matched as a complete normalized value, so embedded,
prefixed, suffixed, multiple, and out-of-domain race-number strings fail closed. The
dedicated contract now also pins a real exact horse-profile capture page-kind rejection,
every visible identity field, malformed/missing/duplicate result cells, exact mutual
result/snapshot membership, incoherent snapshot crosswalks, every finality-container
shape, and the unchanged special-state fail-closed envelope. No result write occurs on
any of those failures.

Supported behavior remains `NORMAL_FINAL_COMPLETE_ONLY`; positive payout-publication
presence remains only a finality predicate, not payout normalization. Capture loading
remains exactly once by exact ID, and one result save occurs only after complete
validation. Existing identity, capture, past-race parser, repository, SQLite, schema,
migration, c4g2a, and c4g2b files remain unchanged. C4h1 is unstarted.
...
diff checks pass. No live HTTP or trusted capture was performed. Stop for independent
