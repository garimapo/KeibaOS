...
Phase 4C-2d3b1i6c1c - Historical input source-record to snapshot assembly preparation
...
c1c is a design-only phase for the smallest pure, fail-closed assembler from a caller-supplied, already validated
...
`validate_historical_input_source_record_set()` must be called first by the future assembler and remains the sole
...
The proposed keyword-only API is:
...
proposed. `source_records` must be an exact tuple and every item an exact committed c1a record. The mapping must be
...
This PREPARE phase changes documentation only and stops at `DRAFT_FOR_REVIEW` after publication for design review.
It does not authorize c1c implementation, test creation, source collection, provider/parser changes, persistence,
or a snapshot builder commit. Current c1b-only tuples are intentionally rejected as incomplete until a future source
phase provides complete official past-race records or exact c1a absence proof for every entry.
